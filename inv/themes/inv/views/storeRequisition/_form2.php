<?php
$data = StoreRequisition::model()->findAll(array("condition" => "sl_no='" . $sl_no . "'"));
if ($data) {
    ?>
<?php
$form = $this->beginWidget('CActiveForm', array(
            'id' => 'store-requisition-form',
            'action' => $this->createUrl('storeRequisition/update'),
            'enableAjaxValidation' => false,
            'enableClientValidation' => true,
            'clientOptions' => array('validateOnSubmit' => true),
        ));
?>
<input type="hidden" name="StoreRequisition[created_time]" value="<?php echo end($data)->created_time; ?>"/>
<input type="hidden" name="StoreRequisition[created_by]" value="<?php echo end($data)->created_by; ?>"/>
<input type="hidden" name="StoreRequisition[sl_no]" value="<?php echo end($data)->sl_no; ?>"/>
<input type="hidden" name="StoreRequisition[max_sl_no]" value="<?php echo end($data)->max_sl_no; ?>"/>
<div class="formDiv">
    <fieldset>
        <legend>Store Requisition Update Form</legend>
            ...
            <table class="checkoutTab" id="tbl">
                ...
                <?php
                    ...
                    foreach($data as $d){
                        ...
                        <tr class='cartList'>
                            <input type='hidden' name='StoreRequisition[id][]' value='<?php echo $d->id; ?>' id="StoreRequisition_<?php echo $sl; ?>"/>
                            <input type='hidden' name='StoreRequisition[item][]' value='<?php echo $d->item; ?>'/>
                            ...
                            <td>
                                <input style='text-align: center;' id='qtyInpt_<?php echo $sl; ?>' myAttr1='qtyInpt_' myAttr2='<?php echo $sl; ?>' type='text' name='StoreRequisition[qty][]' value='<?php echo $d->qty; ?>'/>
                            </td>
        ...
        <div class="rightDiv">
            <table>
                ...
                <tr>
                    ...
                    <td><?php $model->remarks=end($data)->remarks; echo $form->textField($model, 'remarks', array('maxlength' => 255, 'class' => 'coloredInput')); ?></td>
                </tr>
                <tr>
                    <td></td>
                    <td>
                        ...
                        <?php
                        echo CHtml::ajaxSubmitButton('Save', CHtml::normalizeUrl(array('storeRequisition/update', 'sl_no' => end($data)->sl_no)), array(
                            'dataType' => 'json',
                            'type' => 'post',
                            'success' => 'function(data) {
                                $("#ajaxLoader").hide();  
...
//    var newArr=new Array();
    ...
    $(document).ready(function(){
        ...
        $(".resetBtn").click(function(){
            ...
            sl=0;
            $("#store-requisition-form")[0].reset();
        });
    ...
    $("#tbl td input.rdelete").live("click", function () {
        if(confirm("Are you sure you want to delete this record ? The record will be deleted parmanently.")){
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: '<?php echo CController::createUrl('storeRequisition/deleteFromUpdate'); ?>',
                data: { 'id': $("#StoreRequisition_"+idCounter).val() },
                beforeSend:function(){
    ...
    function add(itemsId, itemsName){
        ...
        var appendTxt = "<tr class='cartList'>"+
            "<input type='hidden' name='StoreRequisition[id][]' value='0'>"+
            "<input type='hidden' name='StoreRequisition[item][]' value='"+itemsId+"'>"+
            "<td class='sno' style='text-align: center;'>"+
            slNumber +
            "</td><td style='text-align: left; padding-left:2px;'>" + 
            itemsName +
            "</td><td>" +
            "<input style='text-align: center;' id='qtyInpt_"+sl+"' myAttr1='qtyInpt_' myAttr2='"+sl+"' type='text' name='StoreRequisition[qty][]' value='1'>"+
