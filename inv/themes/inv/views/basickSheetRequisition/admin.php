...
$('.deleteall-button').click(function(){
        var atLeastOneIsChecked = $('input[name=\"basick-sheet-requisition-grid_c0[]\"]:checked').length > 0;
        if (!atLeastOneIsChecked){
                alertify.alert('Please select atleast one record to delete');
        }else if(window.confirm('Are you sure you want to delete the selected records ?')){
                document.getElementById('basick-sheet-requisition-form').action='deleteall';
                document.getElementById('basick-sheet-requisition-form').submit();
        }
});
");
?>
...
?>
<fieldset style="float: left; width: 98%;">
    <legend>Manage Basic Sheet Requisitions</legend>
    ...
    $this->widget('ext.groupgridview.GroupGridView', array(
        'id' => 'basick-sheet-requisition-grid',
        ...
        'columns' => array(
            ...
            array(
                'name' => 'sl_no',
                'header' => 'Edit',
                'value' => 'BasickSheetRequisition::model()->update($data->sl_no)',
                'filter' => '',
            ),
            array(
                'name' => 'sl_no',
                'header' => 'Delete',
                'value' => 'BasickSheetRequisition::model()->delete($data->sl_no)',
                'filter' => '',
            ),
        )
    ));
