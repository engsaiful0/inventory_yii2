...
if ($data) {
    ...
    <div class="formDiv">
        <fieldset>
            <legend>Purchase Procurement Form</legend>
            <div class="scrollable">
                <table class="checkoutTab" id="tbl">
                    <tr>
                        <td colspan="9" style="text-align: left; padding: 6px 0px; font-weight: bold;">
                            Purchase Requisition No: <i><?php echo end($data)->sl_no; ?></i>
                            <br/>Requisition Date: <i><?php echo end($data)->date; ?></i>
                        </td>
                    </tr>
                    <tr>
                    ...
                    ?>
                        ...
                        <script>
                            ...
                            <?php echo $form->hiddenField($model, 'req_id[]', array('value' => $d->id)); ?>
                        ...
                        <td>
                            <ul>
                                ...
                                <?php foreach ($orderSubTypeLocal as $keyLocal => $valueLocal) { ?>
                                    <li class="localOrderSubType" style="display: none;"><input type="radio" class="order_sub_type" name="PurchaseProcurement[order_sub_type]" value="<?php echo $keyLocal; ?>" id="<?php echo $valueLocal; ?>"/><label for="<?php echo $valueLocal; ?>"><?php echo $valueLocal; ?></label></li>
                                <?php } ?>
                                <?php $orderSubTypeImport = Lookup::items2("import"); ?>
                                <?php foreach ($orderSubTypeImport as $keyImport => $valueImport) { ?>
                                    <li class="importOrderSubType" style="display: none;"><input type="radio" class="order_sub_type" name="PurchaseProcurement[order_sub_type]" value="<?php echo $keyImport; ?>" id="<?php echo $valueImport; ?>"/><label for="<?php echo $valueImport; ?>"><?php echo $valueImport; ?></label></li>
    <?php } ?>
                            </ul>
                        </td>
                    </tr>
    ...
    <?php $this->endWidget(); ?>
    <script>
        $("#purchase-procurement-form").submit(function(e){
            if($("#tbl tr").length<=1){
                ...
                e.preventDefault();
            }else if($(".order_sub_type").val()==""){
                alertify.alert("Local/Import Sub Type can not be empty.");
            }else{
                $("#ajaxLoaderMR").show();
            }
        });
        ...
        function add(itemsId, itemsName, itemCost){
            ...
            var appendTxt = "<tr class='cartList'>"+
                ...
                "</td><td>" +
                "<select style='text-align: center;width:80px;' id='name_of_unit_" + sl + "' myAttr1='name_of_unit_' myAttr2='" + sl + "' type='text' name='PurchaseProcurement[name_of_unit][]' value='1'><?php
