...
$('.deleteall-button').click(function(){
        var atLeastOneIsChecked = $('input[name=\"store-inventory-grid_c0[]\"]:checked').length > 0;
        if (!atLeastOneIsChecked){
                alertify.alert('Please select atleast one record to delete');
        }else if(window.confirm('Are you sure you want to delete the selected records ?')){
                document.getElementById('store-inventory-search-form').action='deleteall';
                document.getElementById('store-inventory-search-form').submit();
        }
});
");
?>
<?php if ($model->isNewRecord) { ?>
    <?php echo $this->renderPartial('_form', array('model' => $model, 'itemsModel'=>$itemsModel)); ?>
<?php } ?>
<?php
