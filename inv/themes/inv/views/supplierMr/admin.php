...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('supplier-mr-grid', {
		data: $(this).serialize()
	});
	return false;
});
$('.deleteall-button').click(function(){
        var atLeastOneIsChecked = $('input[name=\"supplier-mr-grid_c0[]\"]:checked').length > 0;
        if (!atLeastOneIsChecked){
                alertify.alert('Please select atleast one record to delete');
        }else if(window.confirm('Are you sure you want to delete the selected records ?')){
                document.getElementById('supplier-mr-search-form').action='deleteall';
                document.getElementById('supplier-mr-search-form').submit();
        }
});
");
?>
<fieldset style="float: left; width: 98%;">
    <legend>Print Payment Receipt</legend>
    <table>
        <tr>
            <td>
                <label>PR No</label>
            ...
            <td>
                <?php
                echo CHtml::ajaxLink(
                        "Preview", Yii::app()->createUrl('supplierMr/mrPreview'), array(
                    'type' => 'POST',
                    'beforeSend' => "function(){
                    ...
                    'href' => Yii::app()->createUrl('supplierMr/mrPreview'),
                    'class' => 'additionalBtn'
                        )
                );
                ?>
...
?>
    ...
    $this->widget('ext.groupgridview.GroupGridView', array(
        ...
        'columns' => array(
            ...
            array(
                'name' => 'bank_id',
                'value' => 'Banks::model()->nameOfThis($data->bank_id)',
                'filter' => CHtml::listData(Banks::model()->findAll(array('order'=>'bank_name ASC')), 'id', 'bank_name'),
            ),
            'cheque_no',
            'cheque_date',
            'paid_amount',
