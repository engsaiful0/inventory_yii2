        ...
        <thead>
            <tr>
                ...
                <td colspan="18" style="padding: 10px;">
                    <?php
                    echo CHtml::link('Make purchase requisition for the selected items', "", // the link for open the dialog
                            array(
                        'class' => 'additionalBtn',
                        'onclick' => "{transferAll();}"));
                    ?>
            ...
            <tr>
                ...
                <th>Stock Qty<br/>(Main Inventory)</th>
                <th>Needed Qty</th>
            </tr>
        </thead>
        <tbody>
            ...
            if ($data) {
                $i = 1;
                foreach ($data as $d):
                        $totalSOQty=  SaleOrder::model()->totalSoQtyOfThisItem($d->id, $store);
                        $totalDelivQty= SellDelvRtn::model()->totalDelivQtyOfThisItem($d->id, $store);
                        
                        $remainingDeliverableQty=($totalSOQty-$totalDelivQty);
                        
                        $availableStock=Inventory::model()->presentStockOfThisItem($d->id, $store);
                        
                        $needQty=($remainingDeliverableQty-$availableStock);
                        
                        if($availableStock < $remainingDeliverableQty){
                    ?>
                    <tr class='
                    <?php
                    if ($i % 2 == 0)
                    ...
                    ?>
                        ...
                        <td>
                            <input 
                                ...
                                attrProdReqQty="<?php echo $needQty; ?>" 
                        ...
                        <td><?php echo $totalSOQty; ?></td>
                        <td><?php echo $totalDelivQty; ?></td>
                        <td><?php echo $remainingDeliverableQty; ?></td>
                        <td><?php echo $availableStock; ?></td>
                        <td style="background-color: #2f7ed8;"><?php echo $needQty; ?></td>
                    </tr>
                    <?php
                        }
                endforeach;
...
<script type="text/javascript">
    ...
    function transferAll(){
        if(jQuery('input[type=checkbox]:checked').length==0){
            ...
            var attrProdReqQtyArr = new Array();
            ...
            $('.chckbxitms').each(function() { //loop through each checkbox
                if(this.checked){
                    ...
                    attrProdReqQtyArr.push($(this).attr("attrProdReqQty"));
                }
            });
                
            $('#dialogTransferSelected').dialog('destroy'); 
            $('#dialogTransferSelected').dialog({ autoOpen: false, resizable: false, title: 'Purchase Requisition', width: 'auto', modal: true }); 
            $('#dialogTransferSelected').dialog('open');
<?php
echo CHtml::ajax(array(
    'url' => array('purchaseRequisition/reqFromSoCreate'),
    'data' => "js:{'attrProdArr':''+attrProdArr+'', 'attrStoreArr':''+attrStoreArr+'', 'attrProdReqQtyArr':''+attrProdReqQtyArr+''}",
    'type' => 'post',
    'dataType' => 'json',
    'beforeSend' => "function(){
