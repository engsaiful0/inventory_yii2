...
class DoubliMachinProductionOutputController extends Controller {
    ...
    public function actionProductionReportView(){
        $startDate = $_POST['DoubliMachinProductionInput']['startDate'];
        $endDate = $_POST['DoubliMachinProductionInput']['endDate'];
        $store = $_POST['DoubliMachinProductionInput']['store'];
        $machine = $_POST['DoubliMachinProductionInput']['machine'];
        $category = $_POST['DoubliMachinProductionInput']['category'];
        $item = $_POST['DoubliMachinProductionInput']['item'];
        ...
        if ($startDate != "" && $endDate != "") {
            ...
            $data = DoubliMachinProductionInput::model()->findAll($criteria);
    ...
    public function actionCreate($sl_no) {
        $data = DoubliMachinProductionInput::model()->findAll(array("condition" => "sl_no='" . $sl_no . "'"));
        if ($data) {
            $model = new DoubliMachinProductionOuput;
            $this->performAjaxValidation($model);
            
            if (isset($_POST['DoubliMachinProductionOuput'])) {
                $model->attributes = $_POST['DoubliMachinProductionOuput'];
                date_default_timezone_set("Asia/Dhaka");
                $todayDate = $_POST['DoubliMachinProductionOuput']['date'];
                $dateForInvNo = str_replace("-", "", $todayDate);
                $thisInvMaxNo = Items::model()->maxValOfThisWithDateCondition("DoubliMachinProductionOuput", 'max_sl_no', 'maxSINo', 'date', $todayDate);
                $slInvNo = $dateForInvNo . $thisInvMaxNo;
                $model->max_sl_no = $thisInvMaxNo;
                $model->production_input_no = $sl_no;
                if ($model->validate()){
                    $productionInputInfo=DoubliMachinProductionInput::model()->findByPk($_POST['DoubliMachinProductionInput']['id'][0]);
            ...
            $this->redirect(array('/doubliMachinProductionInput/adminOutput'));
        }
    }
    ...
    public function actionDeleteAll() {
        if (isset($_POST['production-output-grid_c0'])) {
            ...
            $model_item = new DoubliMachinProductionOuput;
    ...
    public function actionAdmin() {
        $model = new DoubliMachinProductionOuput('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['DoubliMachinProductionOuput']))
            $model->attributes = $_GET['DoubliMachinProductionOuput'];
    ...
    public function loadModel($id) {
        $model = DoubliMachinProductionOuput::model()->findByPk($id);
        if ($model === null)
