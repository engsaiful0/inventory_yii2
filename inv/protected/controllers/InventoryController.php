...
class InventoryController extends Controller {
    ...
    public function actionTransferStock() {
        ...
        $this->render('transferStock', array(
            'model' => $model,
            'itemsModel' => new Items,
        ));
    }
    
    public function actionTransferStockView() {
        $store = $_POST['Inventory']['store'];
        ...
        if ($store != "") {
            ...
            $data2=Inventory::model()->findAll($criteria2);
            
            echo CJSON::encode(array(
                'content' => $this->renderPartial('transferStockView', array(
                    'data' => $data2,
                    'store'=>$store,
                        ), true, true),
            ));
    ...
    public function actionTransferStockCreate() {
        $model = new Inventory;
        $storeIds = "no_store";
        $prodIds = "no_prod";
        $prodAS = "no_av_stck";
                ...
                if ($model->validate()) {
                    if (isset($_POST['Inventory']['item'])) {
                        ...
                        foreach ($_POST['Inventory']['item'] as $tempItems):
                            ...
                            $model3 = new StockTransferHistory;
                            $model3->item = $tempItems;
                            $model3->send_qty = $_POST['Inventory']['stock_in'][$i];
                            $model3->send_date = $_POST['Inventory']['date'];
                            $model3->from_store = $_POST['Inventory']['from_store'];
                            $model3->to_store = $_POST['Inventory']['to_store'];
                            $model3->created_by = Yii::app()->user->getId();
                            $model3->created_time = new CDbExpression('NOW()');
                            $model3->save();
        ...
        if (Yii::app()->request->isAjaxRequest) {
            ...
            echo CJSON::encode(array(
                'status' => 'failure',
                'content' => $this->renderPartial('_formTransferStock', array(
                    'model' => $model,
                    'storeIds' => $storeIds,
                    'prodIds' => $prodIds,
                    'prodAS' => $prodAS,
    ...
    public function actionStockReportView(){
        $startDate = $_POST['Inventory']['startDate'];
        $endDate = $_POST['Inventory']['endDate'];
        $store = $_POST['Inventory']['store'];
        $category = $_POST['Inventory']['category'];
        $item = $_POST['Inventory']['item'];
        ...
        if (isset($_POST['Inventory'])) {
            ...
            if ($valid) {
                ...
                foreach ($_POST['Inventory']['item'] as $tempItems):
                    ...
                    $model->store = $_POST['Inventory']['store'];
                    $model->save();
                    $i++;
                endforeach;
    ...
    public function actionDeleteAll() {
        if (isset($_POST['inventory-grid_c0'])) {
            $del_item = $_POST['inventory-grid_c0'];
            $model_item = new Inventory;
