...
class StoreRequisitionController extends Controller {
    ...
    public function actionStoreReqReportView() {
        $startDate = $_POST['StoreRequisition']['startDate'];
        $endDate = $_POST['StoreRequisition']['endDate'];
        $store = $_POST['StoreRequisition']['store'];
        $department = $_POST['StoreRequisition']['department'];
        $category = $_POST['StoreRequisition']['category'];
        $item = $_POST['StoreRequisition']['item'];
        $reqBy = $_POST['StoreRequisition']['req_by'];
    ...
    public function actionRequisitionPreview() {
        ...
        if ($sl_no) {
            ...
            $data = StoreRequisition::model()->findAll(array('condition' => $condition,));
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['StoreRequisition'])) {
            Yii::app()->clientScript->scriptMap['jquery.js'] = false;
            $valid = $model->validate();
            if ($valid) {
                date_default_timezone_set("Asia/Dhaka");
                $todayDate = $_POST['StoreRequisition']['req_date'];
                $dateForInvNo = str_replace("-", "", $todayDate);
                $thisInvMaxNo = Items::model()->maxValOfThisWithDateCondition("StoreRequisition", 'max_sl_no', 'maxSINo', 'req_date', $todayDate);
                $slInvNo = $dateForInvNo . $thisInvMaxNo;
                $i = 0;
                foreach ($_POST['StoreRequisition']['item'] as $tempItems):
                    $model = new StoreRequisition;
                    $model->item = $tempItems;
                    $model->qty = $_POST['StoreRequisition']['qty'][$i];
                    $model->noOfSack = $_POST['StoreRequisition']['noOfSack'][$i];
                    $model->weghtPerSack = $_POST['StoreRequisition']['weghtPerSack'][$i];
                    $model->department = $_POST['StoreRequisition']['department'];
                    $model->req_by = $_POST['StoreRequisition']['req_by'];
                    $model->approve_by = $_POST['StoreRequisition']['approve_by'];
                    
                    $model->remarks = $_POST['StoreRequisition']['remarks'];
                    $model->req_date = $_POST['StoreRequisition']['req_date'];
                    $model->from_store = $_POST['StoreRequisition']['from_store'];
                    $model->store = $_POST['StoreRequisition']['store'];
                    $model->sl_no = $slInvNo;
    ...
    public function actionUpdate($sl_no) {
        ...
        if (isset($_POST['StoreRequisition'])) {
            ...
            if ($valid) {
                $i = 0;
                foreach ($_POST['StoreRequisition']['item'] as $tempItems):
                    $isExist = StoreRequisition::model()->findByPk($_POST['StoreRequisition']['id'][$i]);
                    if ($isExist) {
                        StoreRequisition::model()->updateByPk($_POST['StoreRequisition']['id'][$i], array(
                            'sl_no'=>$_POST['StoreRequisition']['sl_no'],
                            'max_sl_no'=>$_POST['StoreRequisition']['max_sl_no'],
                            'item'=>$tempItems,
                            'qty'=>$_POST['StoreRequisition']['qty'][$i],
                            'req_by'=>$_POST['StoreRequisition']['req_by'],
                            'from_store'=>$_POST['StoreRequisition']['from_store'],
                            'remarks'=>$_POST['StoreRequisition']['remarks'],
                            'store'=>$_POST['StoreRequisition']['store'],
                            'department'=>$_POST['StoreRequisition']['department'],
                            'req_date'=>$_POST['StoreRequisition']['req_date'],
                            'created_by'=>$_POST['StoreRequisition']['created_by'],
                            'created_time'=>$_POST['StoreRequisition']['created_time'],
                        ...
                        $model->sl_no = $_POST['StoreRequisition']['sl_no'];
                        $model->max_sl_no = $_POST['StoreRequisition']['max_sl_no'];
                        $model->save();
                    }
                    $i++;
                endforeach;
    ...
    public function actionDelete($sl_no) {
        StoreRequisition::model()->deleteAll(array("condition"=>"sl_no='".$sl_no."'"));
    ...
    public function actionDeleteFromUpdate() {
        if(isset($_POST["id"])){
            if (Yii::app()->request->isPostRequest) {
                ...
                StoreRequisition::model()->deleteByPk($_POST["id"]);
    ...
    public function actionDeleteAll() {
        if (isset($_POST['store-requisition-grid_c0'])) {
            ...
            $model_item = new StoreRequisition;
    ...
    public function loadModel($id) {
        $model = StoreRequisition::model()->findByPk($id);
        if ($model === null)
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'store-requisition-form') {
