...
class BasickSheetReqDRController extends Controller {
    ...
    public function actionAdminApprove() {
        $model = new BasickSheetReqDR('searchApprove');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['BasickSheetReqDR']))
            $model->attributes = $_GET['BasickSheetReqDR'];
    ...
    public function actionAdminReturn() {
        $model = new BasickSheetReqDR('searchReturn');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['BasickSheetReqDR']))
            $model->attributes = $_GET['BasickSheetReqDR'];
    ...
    public function actionAllDelivery($sl_no) {
        $model = new BasickSheetReqDR;
        $condition = "sl_no='" . $sl_no . "'";
        $reqInfo = BasickSheetRequisition::model()->findAll(array('condition' => $condition,));
        
        if (isset($_POST['BasickSheetReqDR'])) {
            $model->attributes = $_POST['BasickSheetReqDR'];
            if (Yii::app()->request->isAjaxRequest) {
                Yii::app()->clientScript->scriptMap['jquery.js'] = false;
                $i = 0;
                foreach ($_POST['BasickSheetReqDR']['req_id'] as $tempPOid):
                    if($_POST['BasickSheetReqDR']['d_qty'][$i]!=0){
                        $rInfo = BasickSheetRequisition::model()->findByPk($tempPOid);
                        $m = new BasickSheetReqDR;
                        $m->req_id = $tempPOid;
                        $m->req_no = $rInfo->sl_no;
                        $m->d_qty = $_POST['BasickSheetReqDR']['d_qty'][$i];
                        $m->d_date = $_POST['BasickSheetReqDR']['d_date'];
    ...
    public function actionAllApprove($sl_no) {
        ...
        $reqInfo = BasickSheetReqDR::model()->findAll(array('condition' => $condition,));
        $reqInfoMain=  BasickSheetRequisition::model()->findAll(array('condition' => 'sl_no="'.$sl_no.'"',));
        if (isset($_POST['BasickSheetReqDR'])) {
            $model->attributes = $_POST['BasickSheetReqDR'];
            if (Yii::app()->request->isAjaxRequest) {
                Yii::app()->clientScript->scriptMap['jquery.js'] = false;
                ...
                foreach ($_POST['BasickSheetReqDR']['req_id'] as $tempPOid):
                    ...
                    $stockQtyMainInventory=  BasickSheetInventory::model()->presentStockOfThisItem($rInfo->item, $rInfo->store);
                    if($stockQtyMainInventory>0){
                        $model = $this->loadModel($_POST['BasickSheetReqDR']['id'][$i]);
                        $model->is_approved = 1;
...
//                        $storeInventory->save();
    ...
    public function actionReturn($id) {
        ...
        if (isset($_POST['BasickSheetReqDR'])) {
            ...
            if ($model->save()) {
                ...
                $reqInfo = BasickSheetRequisition::model()->findByPk($model->req_id);
    ...
    public function actionDeleteAll() {
        if (isset($_POST['store-req-dr-grid_c0'])) {
            ...
            $model_item = new BasickSheetReqDR;
    ...
    public function actionAdmin() {
        $model = new BasickSheetReqDR('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['BasickSheetReqDR']))
            $model->attributes = $_GET['BasickSheetReqDR'];
    ...
    public function loadModel($id) {
        $model = BasickSheetReqDR::model()->findByPk($id);
        if ($model === null)
