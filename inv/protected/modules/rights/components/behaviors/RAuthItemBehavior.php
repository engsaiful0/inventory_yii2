...
class RAuthItemBehavior extends CBehavior {
    ...
    public function getRevokePermissionLink(CAuthItem $role) {
        ...
        return CHtml::link(Rights::t('core', 'Revoke'), '#', array(
                ...
                'child' => urlencode($this->owner->name),
					...
					success:function() {
						$('#permissions').load('" . Yii::app()->controller->createUrl('authItem/permissions') . "', { ajax:1 $csrf });
                                                document.getElementById('assignBtn_".urlencode($this->owner->name)."').setAttribute('style', 'background-image: url(" . Yii::app()->theme->baseUrl . "/images/ajax-loader-black.gif); background-repeat: no-repeat; background-position:98% 2px;');
                                                document.getElementById('assignBtn_".urlencode($this->owner->name)."').setAttribute('style', 'background-image: unset; background-repeat: no-repeat; background-position:98% 2px;');
					},
                                        beforeSend:function(){
                                                document.getElementById('revokeBtn_".urlencode($this->owner->name)."').setAttribute('style', 'background-image: url(" . Yii::app()->theme->baseUrl . "/images/ajax-loader-black.gif); background-repeat: no-repeat; background-position:98% 2px;');
                                        }
				});
			...
			",
            'class' => 'revoke-link',
            'id'=>'revokeBtn_'.urlencode($this->owner->name),
        ));
    }
    ...
    public function getAssignPermissionLink(CAuthItem $role) {
        ...
        return CHtml::link(Rights::t('core', 'Assign'), '#', array(
            'onclick' => "
                ...
                'child' => urlencode($this->owner->name),
					...
					success:function() {
						$('#permissions').load('" . Yii::app()->controller->createUrl('authItem/permissions') . "', { ajax:1 $csrf });
                                                document.getElementById('revokeBtn_".urlencode($this->owner->name)."').setAttribute('style', 'background-image: url(" . Yii::app()->theme->baseUrl . "/images/ajax-loader-black.gif); background-repeat: no-repeat; background-position:98% 2px;');
                                                document.getElementById('revokeBtn_".urlencode($this->owner->name)."').setAttribute('style', 'background-image: unset; background-repeat: no-repeat; background-position:98% 2px;');
					},
                                        beforeSend:function(){
                                                document.getElementById('assignBtn_".urlencode($this->owner->name)."').setAttribute('style', 'background-image: url(" . Yii::app()->theme->baseUrl . "/images/ajax-loader-black.gif); background-repeat: no-repeat; background-position:98% 2px;');
                                        }
			...
			",
            'class' => 'assign-link',
            'id'=>'assignBtn_'.urlencode($this->owner->name),
        ));
    }
