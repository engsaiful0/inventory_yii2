...
class DoubliMachinProductionInput extends CActiveRecord {
    ...
    public function tableName() {
        return 'doubli_machin_production_input';
    }
    ...
    public function rules() {
        ...
        return array(
            array('input_date,approved_by,doubli_producton_input_by,length,width,thickness,unit_of_distance,qty,qty_kg,ref_no,subj,item', 'required', 'on'=>'update'),
            array('max_sl_no, created_by, updated_by, is_verified', 'numerical', 'integerOnly' => true),
            array('order_qty,return_qty,return_qty_kg', 'numerical'),
            array('sl_no, ref_no, subj','length', 'max' => 255),
            array('input_date,time, created_time,date, updated_time', 'safe'),
            // The following rule is used by search().
            // Please remove those attributes that should not be searched.
            array('id, sl_no,store, max_sl_no, ref_no,length,width,thickness,unit_of_distance, input_date,startDate,endDate,track_no,date,startTime,endTime,machine, subj,store, created_by, created_time, updated_by, updated_time, is_verified', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function attributeLabels() {
        return array(
            ...
            'return_qty_kg'=>'Return Qty(kg)',
            'qty'=>'Qty',
            'qty_kg'=>'Qty(Kg)',
            'doubli_producton_input_by'=>'Doubli Production Input By',
    ...
    public function search() {
        ...
        $data = DoubliMachinProductionInput::model()->findAll($criteria2);
        $req_id = array();
        if ($data) {
            foreach ($data as $d):
                $req_id[] = $d->id;
        ...
        $criteria->compare('unit_of_distance', $this->unit_of_distance);
    ...
    public function returnQty($sl_no){
        echo CHtml::link(CHtml::image(Yii::app()->theme->baseUrl . '/images/returnBtnBg.png'), array('/doubliMachinProductionInput/returnQty', 'sl_no' => $sl_no));
    }
    

    
    public function output($sl_no) {
        echo CHtml::link(CHtml::image(Yii::app()->theme->baseUrl . '/images/outputBtnBg.png'), array('/doubliMachinProductionOutput/create', 'sl_no' => $sl_no), array('target'=>'_blank'));
    }
    
    public function wastage($sl_no) {
        echo CHtml::link(CHtml::image(Yii::app()->theme->baseUrl . '/images/wastageBtnBg.png', '',array('width'=>64)), array('/doubliMachinProductionWastage/create', 'sl_no' => $sl_no), array('target'=>'_blank'));
    }
